    ...
    return (
        <aside className="z-20 hidden w-64 overflow-y-auto bg-white dark:bg-gray-800 md:block flex-shrink-0">
            <div className="py-4 text-gray-500 dark:text-gray-400">
                ...
                <ul className="mt-6">
                    {NavAccount.map((item) => (
                        <li key={item?.name} className={`relative px-6 py-3 text-gray-800`}>
                            {item?.head ? (
                                <>
                                    {router.pathname === item?.href ? <span className={`absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg`} /> : ''}
                                    <button
                                    ...
                                    {openMenu === item?.name && (
                                        <ul className={`transition-opacity duration-300 ease-in-out opacity-100 max-h-xl p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900`}>
                                            {item?.loopPage?.map((subItem) => (
                                                <li key={subItem?.name} className="px-4 py-2 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
                                                    <Link href={subItem?.href} className="w-full">{subItem?.name}
                                                    </Link>
                                                </li>
                                            ))}
                                        </ul>
